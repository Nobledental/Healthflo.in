...
export default function Hero() {
  ...
  return (
    <section className="flex flex-col items-center relative pb-6 overflow-hidden w-full transition-colors duration-1000">
      ...
      <div className="absolute top-0 left-0 w-full flex justify-between items-center px-4 md:px-8 z-20">
        <span className="text-[11px] md:text-[13px] font-semibold uppercase tracking-widest text-slate-700">
          Excellence
        </span>
        <div className="flex items-center gap-3">
          <div className="px-3 py-1 md:px-4 md:py-1.5 rounded-full text-[12px] md:text-[13px] font-semibold flex items-center justify-center border shadow-sm backdrop-blur-md bg-white/70 border-white/50 text-slate-800">
      ...
      <div className="w-full max-w-7xl flex flex-col items-center text-center z-30 px-4 sm:px-6 lg:px-12 mb-2 transition-opacity duration-500 pt-16 md:pt-20">

        {/* H1 */}
        <h1
          className="text-[38px] sm:text-[52px] md:text-[64px] lg:text-[76px] font-extrabold leading-[1.1] tracking-tighter z-10 mb-2 max-w-4xl mx-auto flex items-center justify-center gap-0 flex-wrap"
        >
        ...
        <div className="hidden sm:flex flex-wrap items-center justify-center gap-3 md:gap-8 mb-4 mt-1">
          {[
            ...
            <div key={i} className="flex items-center gap-1.5">
              <span className="font-extrabold text-slate-900 text-[15px] md:text-[18px] leading-none">{s.value}</span>
              <span className="text-slate-500 text-[11px] md:text-[13px]">{s.label}</span>
        ...
        <div className="w-full max-w-[100vw] overflow-hidden relative mb-5 flex items-center group [mask-image:linear-gradient(to_right,transparent,black_10%,black_90%,transparent)] py-3 md:py-5 border-y border-slate-200/50">
          <div className="flex whitespace-nowrap items-center" style={{ animation: "slide-marquee 25s linear infinite", width: "max-content" }}>
            {[...Array(2)].map((_, loopIdx) => (
              <div key={loopIdx} className="flex items-center">
                {treatments.map((t, i) => (
                  <div key={i} className="inline-flex items-center px-5 md:px-8 group/item cursor-pointer">
                    <span className="text-slate-600 font-bold text-[11px] md:text-[14px] lg:text-[16px] tracking-widest uppercase group-hover/item:text-[#0a84ff] transition-colors duration-300">
                      {t}
                    </span>
                    <div className="w-[1px] h-6 md:h-8 bg-slate-200 ml-10 md:ml-16 opacity-50"></div>
                  </div>
                ))}
              </div>
            ))}
        ...
        {/* AI Health Assistant App Banner — responsive layout */}
        <div className={`w-full mx-auto bg-gradient-to-r from-[#d4e9ff] to-[#bed8ff] rounded-[1.25rem] md:rounded-[1.5rem] p-4 md:px-8 md:py-5 flex flex-col xl:flex-row items-start xl:items-center justify-between gap-4 transition-all duration-500 shadow-sm border border-blue-200/50 ${powerSavingMode ? "opacity-50 pointer-events-none scale-95" : "opacity-100 scale-100"}`}>
          
          <div className="text-left flex-1 min-w-0 flex flex-col gap-1.5 items-start">
            ...
            <p className="text-[14px] md:text-[16px] lg:text-[18px] font-medium text-slate-700 leading-snug">
              Your personal AI health assistant, <span className="text-[#0a84ff]">bringing expert care to your hands.</span>
            </p>
          </div>

          <div className="flex flex-row items-center justify-start md:justify-end gap-2 md:gap-3 flex-shrink-0 w-full xl:w-auto">

            {/* Try in Web Button */}
            <a
              href="#web-app"
              className="flex items-center gap-2 bg-[#0a84ff] hover:bg-blue-600 text-white rounded-xl px-3 py-2 md:px-4 transition-all shadow-sm group"
            >
              <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" className="w-5 h-5 group-hover:rotate-12 transition-transform">
              ...
              <div className="flex flex-col text-left">
                ...
                <span className="text-[13px] md:text-[14px] font-bold leading-none tracking-tight">TRY IN WEB</span>
              </div>
            </a>
            ...
            <a
              ...
              className="flex items-center gap-2 bg-white hover:bg-slate-50 text-slate-800 rounded-xl px-3 py-2 md:px-4 transition-all shadow-sm border border-slate-100 group"
            >
              <svg viewBox="0 0 48 48" className="w-5 h-5 group-hover:scale-110 transition-transform" fill="none">
              ...
              <div className="flex flex-col text-left">
                ...
                <span className="text-[13px] md:text-[14px] font-bold leading-none tracking-tight">Google Play</span>
              </div>
            </a>
            ...
            <a
              ...
              className="flex items-center gap-2 bg-white hover:bg-slate-50 text-slate-800 rounded-xl px-3 py-2 md:px-4 transition-all shadow-sm border border-slate-100 group"
            >
              <svg viewBox="0 0 814 1000" className="w-4 h-5 group-hover:scale-110 transition-transform" fill="#1a1a1a">
              ...
              <div className="flex flex-col text-left">
                ...
                <span className="text-[13px] md:text-[14px] font-bold leading-none tracking-tight">App Store</span>
              </div>
            </a>
